...
data class Chicken(
    ...
    val timeAliveStr: String
        get() {
            val allMonths = timeAliveInDays
            if (allMonths <= 0) return "0"
            val years = allMonths / 12
            val months = allMonths % 12
            val monthsStr = if (months > 1) "$months months"
            else "1 month"
            val yearsStr = if (years > 1) "$years years"
            else "1 year"
    ...
    val healthPercent: Int
        get() = ((health / maxHealth) * 100).roundToInt()
    ...
    fun getCopyAfterTimePass(secondsPassed: Int): Chicken {
        ...
        return copy(
            timeAlive = timeAlive + if (newHealth > 0) seconds else 0,
